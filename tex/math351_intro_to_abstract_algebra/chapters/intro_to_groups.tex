...
\begin{dfnbox}{Cyclic group, generator}{}
    A group $G$ is \dfntxt{cyclic} if there exists $a \in G$ where every element is a power of $a$. We say $a$ is a \dfntxt{generator} for $G$ and write $G = \alg{a}$.
\end{dfnbox}
...
\begin{dfnbox}{Congruence modulo a group}{}
    For $H \leq G$ and $a,b \in G$, $a$ is \dfntxt{congruent to} $b$ modulo $H$ if $a^{-1}b \in H$. This is written as $a \equiv b \pmod{H}$.
\end{dfnbox}
...
\begin{dfnbox}{Left coset}{}
    For $H \leq G$ and any $a \in G$, the \dfntxt{left coset} of $a$ with respect to $G$ is set $\{ah : h \in H\}$, written as $aH$. We also call $aH$ a left coset of $H$ in $G$.
\end{dfnbox}

\begin{thmbox}{Left cosets partitioning}
    For $H \leq G$, the left cosets of $H$ in $G$ partition $G$.
\end{thmbox}

\begin{dfnbox}{Index}{}
    For $H \leq G$, the \dfntxt{index} of $H$ in $G$ is the number of distinct left cosets of $H$ in $G$, written as $[G : H]$.
\end{dfnbox}
