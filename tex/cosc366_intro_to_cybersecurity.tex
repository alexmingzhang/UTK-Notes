...
\begin{enumerate}
    ...
    \item \dfntxt{Kerckhoff's Principle} -- security should only come from the secrecy of the crytographic key material, NOT the algorithm or code.
...
\begin{dfnbox}{Asymmetric Encryption, Public Key, Private Key}{}
    \dfntxt{Asymmetric encryption} utilizes two cryptographic keys: a \dfntxt{public key} which encrypts data, and a \dfntxt{private key} which decrypts data.
\end{dfnbox}
...
\begin{dfnbox}{Hybrid Encryption, Key Wrap}{}
    \dfntxt{Hybrid encryption} incorporates both symmetric key and public key encryption. We encrypt our message using a symmetric key, and we send both the encrypted message and the symmetric key encrypted using the other party's public key. This encrypted key is called a \dfntxt{key wrap}.
\end{dfnbox}
