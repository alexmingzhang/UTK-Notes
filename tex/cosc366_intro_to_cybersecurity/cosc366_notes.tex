...
\begin{exbox}{Salted Hashing}{}
    ...
    \begin{itemize}[noitemsep]
        ...
        \item Website generates a random \dfntxt{salt}
...
\begin{dfnbox}{Subject, Action, Object}{}
    A \dfntxt{subject} is any entity (such as a user or process) trying to take some \dfntxt{action} such as read, write, execute, start, shutdown, etc. The \dfntxt{object} receives some action, such as some memory, files, services, or devices.
\end{dfnbox}
...
\begin{dfnbox}{Policy, Reference Monitor}{}
    \dfntxt{Policies} define what is allowed, usually parameterized by subject, action, and object. \dfntxt{Reference monitors} enforce a policy. It only allows a subject to execute an action on an object if that action conforms with the policy.
\end{dfnbox}
...
\begin{itemize}
    \item \dfntxt{Complete Mediation} -- the only way to access objects is through the reference monitor; ensures access logs are complete
    \item \dfntxt{Tamper-proof} -- also includes tamper-proof logs
...
\begin{dfnbox}{Capability-Based Access Control}{}
    \dfntxt{Capability-Based Access Control} reference monitor issues bearer tokens for given permissions. Anyone with the bearer token can take the action on the given object.
\end{dfnbox}
