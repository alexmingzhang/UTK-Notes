...
\begin{dfnbox}{Discretionary Access Control, Mandatory Access Control}{}
    In \dfntxt{discretionary access control}, users are responsible for assigning access control rules for their files. In \dfntxt{mandatory access control}, the policy administrator sets all the access control rules; usually tied to role-based access control (RBAC).
\end{dfnbox}
...
How to fix tihs:
\begin{itemize}
    \item Application logic race conditions: use appropriate locking (mutexes, semaphores, etc.)\todo{What is this}
...
\begin{enumerate}[noitemsep]
    ...
    \item \dfntxt{Propagation strategy:} how the malware spreads to other users
    \item \dfntxt{Trigger condition:} how long the malware waits to take effect
    \item \dfntxt{Payload:} what does the malware actually do?
\end{enumerate}

\begin{dfnbox}{Computer Virus}{}
    A \dfntxt{computer virus} is malware that can infect other programs or files by modifying them to include a possibly evolved copy of itself.
\end{dfnbox}
...
\begin{itemize}
    ...
    \item \dfntxt{Polymorphic encrypted payloads:} we use the same encryption, but the virus changes its decryption methods. More advanced, it could use a compiler to create new implementations on-the-fly.
