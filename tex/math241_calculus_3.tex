...
\begin{exbox}{Gradient}{}
    $f(T,L,\rho) = \frac{1}{2L} \sqrt{\frac{T}{\rho}}$
...
\begin{dfnbox}{Directional Derivative}{}
    The \dfntxt{directional derivative} of $f(x,y,z)$ in the direction of $\vec{u}$ is defined as:
    \[ \nabla f(x,y,z) \cdot \vec{u} \]
\end{dfnbox}

The dot product of the gradient $\nabla f(x,y,z)$ with a unit vector $\frac{\vec{a}}{\norm{\vec{a}}}$
...
\begin{dfnbox}{Vector Projection}{}
    Given $\vec{a}$ and $\vec{b}$ that are non-zero vectors, the \dfntxt{vector projection} of $\vec{b}$ over the vector $\vec{a}$ is denoted by:
\end{dfnbox}
